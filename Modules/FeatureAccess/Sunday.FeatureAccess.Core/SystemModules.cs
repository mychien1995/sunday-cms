using System;
using System.Collections.Generic;
using System.Text;

namespace Sunday.FeatureAccess.Core
{
    public static class SystemModules
    {
        public static class UsersManagement
        {
            public const string DisplayName = "Users Management";
            public const string Code = "UM";
        }
        public static class OrganizationsManagement
        {
            public const string DisplayName = "Organizations Management";
            public const string Code = "OM";
        }
        public static class RolesManagement
        {
            public const string DisplayName = "Roles Management";
            public const string Code = "ORM";
        }
    }
}
